...
public class SolicitudesServicesImple implements SolicitudesServices {
    ...
    // Nuevo: sobrecarga para recibir múltiples IDs de elementos desde el controlador
    @Transactional
    public SolicitudesDto guardar(SolicitudeCreateDto dto, java.util.List<Long> idsElem) {
        if (dto == null) {
            throw new IllegalArgumentException("DTO de creación es obligatorio");
        }
        Solicitudes solicitudes = solicitudesMapper.toSolicitudesFromCreateDto(dto);
        if (solicitudes.getEstadosolicitud() == null) {
            solicitudes.setEstadosolicitud((byte) 2);
        }
        if (idsElem != null && !idsElem.isEmpty()) {
            for (Long idElem : idsElem) {
        ...
        } else if (dto.getId_elem() != null && (solicitudes.getElemento() == null || solicitudes.getElemento().isEmpty())) {
            // Fallback: un solo elemento si no llegó arreglo por parámetro
            Elementos elemento = elementosRepository.findById(dto.getId_elem())
                .orElseThrow(() -> new EntityNotFoundException("Elemento no encontrado"));
            Elemento_Solicitudes es = new Elemento_Solicitudes();
            es.setSolicitudes(solicitudes);
